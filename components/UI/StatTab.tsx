...
export const StatTab = ({ stat_value, stat_name,limit }: statTabInterface) => {
  const base_limit=10;
  return(<div className={'w-full h-5 deactive-gradient rounded-full transition-all'}>
    <div style={{ width: `${stat_value * 100/limit}%`,minWidth:`5%` , background: `${whichStat(stat_name)}` }} className={'h-full rounded-full flex justify-end items-center transition-all duration-500 ease-in-out'}>
